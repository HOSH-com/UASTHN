...
if __name__ == "__main__":
    ...
    satellite = load_and_preprocess_satellite(
        'js_datasets/Dehat/satellite/2.tif', model.database_size).to(device)
    thermal = load_and_preprocess_thermal(
        'js_datasets/Dehat/thermal/2_2.tif', model.resize_width).to(device)


    ue_shift_crops_types = model.model_config.get('ue_shift_crops_types', 'grid')
    with torch.no_grad():
        ...
        four_pred, std_pred = model.forward_with_uncertainty(
            ...
            ue_shift_crops_types=ue_shift_crops_types)
